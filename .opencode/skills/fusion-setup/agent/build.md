...
- Your `bash` is allowlisted to verification and git commands (`npm run lint`, `npm test`, `git diff`, `git status`, `git log`, `git show`, `git add`). `git commit` and `git push` run only with per-command user approval, and force/mirror/delete-ref pushes are blocked outright. File-writing commands and other git state-modifying commands are blocked.
