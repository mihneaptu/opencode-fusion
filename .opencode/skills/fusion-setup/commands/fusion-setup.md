...
If arguments are empty, run the full interactive setup: offer the bundled subscription profiles first, then ask which model to use for each role (main/build, sidekick, explore, and the optional research/design/reviewer/vision specialists), then write the global config and install the agent prompts as the skill describes. If arguments name a subscription or profile (for example "opencode-go" or "use my Claude subscription"), jump straight to the skill's Step 0 confirmation for that profile. If arguments describe a change (for example "reconfigure sidekick" or "change explore model"), treat them as the specific change the user wants and jump to the relevant part of the skill instead of re-running the whole interview.
