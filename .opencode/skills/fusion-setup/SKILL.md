...
5. Check whether the project is a Node/JavaScript one. The bundled `build.md`, `plan.md`, and `reviewer.md` allowlist `npm test`, `npm run lint`, `npm run build`, `npx tsc --noEmit`, and `npx vitest run` as their verification commands. On any other toolchain those never match, so the main agent cannot verify its own work and will be denied on the command it should be running. Tell the user which entries to swap in for their stack - `pytest` and `ruff check` for Python, `cargo test` and `cargo clippy` for Rust, `go test` and `go vet` for Go, `make test` and `make lint` for Make-driven builds - keeping `"*": "deny"` first, keeping the commands read-only rather than reformatting, and adding a `deny` for any autofix or snapshot-update flag the way the bundled list denies `npm run lint --fix` and `npx vitest run -u`. The git half of the allowlist needs no change.
