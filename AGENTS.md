...
- The team: `build` and `plan` are primary (Fusion-aware, cannot edit); `sidekick` executes; `explore`/`research` are read-only; `design` edits UI; `reviewer` audits diffs read-only. Most subagents carry `task: allow`, but `vision` is a deliberate leaf (`task: deny`). Nested delegation (a subagent spawning another subagent) is verified at runtime - a build -> sidekick -> explore chain was confirmed in the session DB (opencode.db), with each child session's parent_id linking back to its spawner.
...
- `plan.md` overrides opencode's built-in plan agent so plan mode stays Fusion-aware (explores read-only, does not execute, cannot commit). Without it, plan mode would be vanilla opencode.
- opencode (tested on 1.17.16) loads agent prompts from the SINGULAR `~/.config/opencode/agent/` directory. The plural `agents/` is not used here. The skill installs to the singular path; do not create an `agents/` dir (it is the confusing lookalike warned about elsewhere).
- The sidekick's `task` permission is a fine-grained graph (`{"*": deny, "explore": allow, "research": allow}`), so it can spawn only read-only searchers, not other executors. This bounds nested delegation without needing budget/depth caps.
- This opencode version's agent schema supports `steps` (max agentic iterations, a cost cap) but NOT `task_budget` or `level_limit` (not AgentConfig fields in 1.17.16). Loop protection therefore comes from the `permission.task` graph, not from budget/depth fields - do not add those to agent config expecting them to work.
- Cross-vendor review is built in: the main agent (Opus via kirocc) and the sidekick (Grok 4.5 via progrok) are different model families, so every sidekick diff gets an independent-family review by the main agent. Note also that dynamic mid-session model routing (Devin Fusion's second technique) is NOT possible in opencode - it needs Devin's closed harness - and is an explicit non-goal here.
